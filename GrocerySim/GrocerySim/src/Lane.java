    ...
    public Event getEvent() {
        ...
        if (this instanceof ExpressLane) {
            for (int i = 0; i < this.size(); i++) {
                ...
                // adds the customers time to checkout plus how long they had to wait for the customers in front of them to check out
                double frontOfTheLine = this.get(i).getCustomer().getFinishShoppingTime() + waitTime;
                // adds the customers wait time and the time they finished shopping to determine when they get to the front of the line
                ...
                // if the customers are in the same lane then it sums the previous customers required time to checkout
