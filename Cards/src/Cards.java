...
public class Cards {
    ...
    public static void humanMove(Player player, Deque<Card> discardPile, ArrayList<Card> drawPile) {
        ...
        if (discardPile.isEmpty()) {
            ...
            } // continually asks for input while the input given is not 1 2 3 4 or 5, once a
              // valid input is given valid is set to true
            ...
            while (!valid) {
                try {
                    ...
                    if (choice == 1 || choice == 2) {
                        ...
                        System.out.println(
                                "Do you want to pick up the " + discardPile.peek() + "(1)" + " or draw a card (2)");
                    }
            ...
            } // continually asks for input while the input given is not 1 or 2, once a valid
              // input is given valid is set to true
            if (choice == 1) {
            ...
            } // if the player chooses two then they draw from the draw pile if the drawpile
              // is empty the discardpile becomes the drawpile
            player.showHand();
            System.out.println("Which one do you want to discard?");
            while (valid) {
                try {
              ...
              // valid input is given valid is set to false
            player.discard(choice, discardPile);
        } // if the discard pile is empty they automatically draw from the draw pile
          // otherwise they can draw from either the draw pile or discard pile
        if (player.winner()) {
            player.showHand();
            System.exit(0);
        } // the winning player is outputed along with their hand and then the game is
          // exited
    ...
    public static void computerMove(Player computer, Deque<Card> discardPile, ArrayList<Card> drawPile) {
        if (discardPile.isEmpty()) {
            ...
            if (computer.getHand().get(0).getRank().equals(discardPile.peek().getRank())
                    || computer.getHand().get(1).getRank().equals(discardPile.peek().getRank())
                    || computer.getHand().get(2).getRank().equals(discardPile.peek().getRank())
                    || computer.getHand().get(3).getRank().equals(discardPile.peek().getRank())) {
                computer.computerDraw(discardPile);
                // if the card on top of the discard pile matches one of their cards they pick
                // it up
            } else if (Math.random() < .4) {
                computer.computerDraw(discardPile);
                // if the random is below .4 they draw a card from the discard pile, to avoid
                // the discard pile stagnating
            } else {
                if (drawPile.isEmpty()) {
                    drawPile.addAll(discardPile);
                    Collections.shuffle(drawPile);
                  ...
                  // they draw a card from the new draw pile otherwise they just
    ...
    }// if the player object is a computer then it performs a computers move
     // otherwise the player object performs a human players move
    ...
    public static void main(String[] args) {
        ...
        } // continually asks for input while the input given is not 1 2 3 or 4, once a
          // valid input is given valid is set to false
        for (int i = 0; i < 4; i++) {
            if (i < numPlayers) {
        ...
        } // creates the players for the game, adds the indicated number of human players
          // and then adds the remaining players as computers
